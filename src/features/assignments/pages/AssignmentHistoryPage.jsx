/**
 * Navix Assignments — AssignmentHistoryPage
 * --------------------------------------------------------------------------
 * Historique des affectations passées (terminées / annulées) : recherche,
 * filtre de statut, tri, pagination locale et états chargement / erreur /
 * vide.
 */
...
import { useAssignmentHistoryData } from '../hooks';
import { AssignmentSearchBar, AssignmentHistoryTable, EmptyAssignmentState } from '../components';

const AssignmentHistoryPage = () => {
  const navigate = useNavigate();

  const history = useAssignmentsStore((state) => state.history);
  const isLoading = useAssignmentsStore((state) => state.isLoading);
  const error = useAssignmentsStore((state) => state.error);
  const fetchHistory = useAssignmentsStore((state) => state.fetchHistory);
  ...
  const [sort, setSort] = useState({ by: 'endDate', direction: 'desc' });
  ...
  const { items, totalItems, totalPages } = useAssignmentHistoryData({
    history,
    search,
    status,
    sort,
  ...
  const handleReset = () => {
    setSearch('');
    setStatus('');
    setSort({ by: 'endDate', direction: 'desc' });
    setPage(1);
  };
  ...
  return (
    <PageContainer>
      <Helmet>
        <title>Historique des affectations — Navix Management</title>
      </Helmet>

      <PageHeader
        title="Historique des affectations"
        subtitle="Affectations terminées et annulées : dates, kilométrage et durée."
      ...
      <div className="d-flex flex-wrap align-items-center gap-2 mb-3">
        <div className="d-flex flex-wrap align-items-center gap-2">
          ...
          >
            ...
            {ASSIGNMENT_STATUS_VALUES.map((value) => (
              <option key={value} value={value}>
                {ASSIGNMENT_STATUSES[value].label}
              </option>
            ))}
          </select>
          <select
      ...
      {isLoading && history.length === 0 ? (
        ...
        <EmptyAssignmentState hasQuery={hasActiveFilters} onReset={hasActiveFilters ? handleReset : undefined} variant="history" />
      ) : (
        <>
          <div className="card">
