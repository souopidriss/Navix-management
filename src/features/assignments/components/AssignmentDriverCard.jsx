/**
 * Navix Assignments — AssignmentDriverCard
 * --------------------------------------------------------------------------
 * Carte du chauffeur associé à une affectation (page de détail) : photo,
 * nom complet, code employé, statut, disponibilité et téléphone.
 *
 * Props :
...
import './AssignmentDriverCard.css';

const AssignmentDriverCard = ({ driver, onView }) => {
  const availability = driver ? getDriverAvailability(driver.availability) : null;

  return (
    <article className="card h-100 navix-assignment-driver-card">
      <div className="card-body">
        <div className="d-flex align-items-center gap-3 min-w-0">
          <DriverAvatar src={driver?.photo} fullName={driver?.fullName} size="lg" />
          <div className="min-w-0">
            <h3 className="navix-assignment-driver-card__name">{driver?.fullName ?? 'Chauffeur'}</h3>
            <p className="navix-assignment-driver-card__code mb-0">{driver?.employeeCode ?? '—'}</p>
          </div>
        </div>

        {driver && (
          <div className="navix-assignment-driver-card__badges">
        ...
        <dl className="navix-assignment-driver-card__meta">
          <div>
            <dt>
...
export default AssignmentDriverCard;
