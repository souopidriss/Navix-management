/**
 * Navix Dashboard — MaintenanceTrendChart
 * --------------------------------------------------------------------------
 * Graphique en barres (CSS pur) du coût mensuel des entretiens sur les
 * 6 derniers mois.
 */
import './DashChart.css';

const MaintenanceTrendChart = ({ data = [], formatValue, title = 'Coût entretiens (6 mois)' }) => {
        ...
        {data.map((item) => {
          ...
          return (
            <div key={item.month} className="navix-dash-chart__col">
              ...
              <span className="navix-dash-chart__bar navix-dash-chart__bar--warning" style={{ height: `${height}px` }} />
