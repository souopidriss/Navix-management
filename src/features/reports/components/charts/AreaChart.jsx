/**
 * Navix Reports — AreaChart
 * --------------------------------------------------------------------------
 * Graphique en aires (SVG natif, aucune bibliothèque externe) : courbe avec
 * remplissage translucide sous la ligne. Idéal pour les tendances sur une
 * période (sommes mensuelles, volumes, coûts…).
 *
 * Props :
 *   data         : { labels: string[], datasets: [{ key, label, values, variant }] }
 *   height       : hauteur du plot en pixels            (défaut : 240)
 *   title        : libellé accessible (aria-label)      (défaut : 'Graphique en aires')
 *   showDots     : affiche les points sur la courbe      (défaut : false)
...
const AreaChart = ({
  ...
  const { x, y, linePath, areaPath, gridLines, viewW, viewH } = computeLineGeometry(labels);
      ...
      <div className="navix-report-chart__svg-wrap" style={{ height }} role="img" aria-label={title}>
        <svg viewBox={`0 0 ${viewW} ${viewH}`} preserveAspectRatio="none" className="navix-report-chart__svg" aria-hidden="true">
          ...
          {datasets.map((dataset) => {
            ...
            return (
              <path key={`area-${dataset.key}`} d={areaPath(dataset.values || [], max)} fill={color} className="navix-report-chart__area" />
            );
          })}
          {datasets.map((dataset) => (
            <path
              key={`line-${dataset.key}`}
              d={linePath(dataset.values || [], max)}
              stroke={VARIANT_COLORS[dataset.variant] || VARIANT_COLORS.primary}
              className="navix-report-chart__line"
            />
          ))}
          {showDots &&
            datasets.map((dataset) =>
